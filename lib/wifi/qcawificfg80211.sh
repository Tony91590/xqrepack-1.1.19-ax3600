...
detect_qcawificfg80211() {
	...
	for dev in $(ls -d wifi* 2>&-); do
		...
		# mywifimac=$(macaddr_add "$mac" 2+$devidx)

		# mywifimac=$(cat /sys/class/net/${dev}/address)
...
config wifi-iface
	...
	option network	lan
	option mode	ap
	option ssid	OpenWrt_$(printf $mywifimac | awk -F ":" '{print $4""$5""$6 }'| tr a-z A-Z)${ssid_pad}
	option wmm      1
